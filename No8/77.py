...
def train(model, dataloaders, lr, epochs):
    ...
    for e in range(epochs):
        ...
        tloss, tacc, vloss, vacc, epoch_time = train_epoch(model, dataloaders, optimizer, criterion)
        torch.save({
            'epoch': e,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict()
        }, 'checkpoint/77checkpoint{e+1}.pth')
...
def main():
    ...
    for batchs in batch_sizes:
        ...
        train_loss, train_acc, valid_loss, valid_acc, avg_time = train(model_SN, dataloaders, lr, epochs)
