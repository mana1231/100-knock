...
class RNN(nn.Module):
    def __init__(self, vocab_size, emb_size, padding_idx, hidden_size, output_size, num_layers=1, emb_weights=None):
        ...
        self.rnn = nn.LSTM(emb_size, hidden_size, num_layers, batch_first=True, bidirectional=True) # add bidirectional=True
        self.out = nn.Linear(hidden_size*2, output_size) # change h_size -> h_size*2
...
def main():
    ...
    model = RNN(vocab_size, emb_size, PAD, hidden_size, output_size, num_layers=num_layers, emb_weights=emb_weights)
